...
- A Telegram Mini App now exists (`bot/miniapp/`) — see the dedicated "Mini App" section below. This supersedes the earlier "no Mini App, deliberate choice" stance: chat-only worker screens (text + emoji + inline keyboards) remain fully functional and are not being removed, but a colorful in-Telegram WebView is also available for every role.
- Routes live under `/api/*` (`web/src/routes/`), each protected by the `requireAuth` middleware from `auth.js`.

### Mini App (`bot/miniapp/`) — a colorful in-Telegram surface, backend stays Python

A Telegram Mini App (WebView opened from an inline `web_app` button, `keyboards/miniapp_kb.build_miniapp_button()`) for all six roles, added because Telegram chat messages/buttons cannot render color, cards, or charts — only bold/italic/monospace text and OS-styled gray buttons. Unlike `web/`, this is **not** a separate Node app: `bot/main.py` runs an `aiohttp.web` server (`miniapp/server.py`) *inside the same process* as the polling loop via `asyncio.gather()` — no new deploy target, no logic duplicated in a second language. Every endpoint calls the exact same `services/*.py` functions the chat handlers use (`task_service`, `timer_service`, `penalty_service`, `employee_service`, `sales_service`, `financial_service`, `stats_service`) — the one deliberate exception to the "hand-duplicate in the other language" pattern `web/` established, chosen specifically because these are **write** paths with real side effects (Trello card moves, KPI penalties); duplicating that logic in JS was rejected as the highest-risk kind of divergence bug.

- **Auth**: Telegram WebApp `initData` (HMAC-SHA256, keyed by `SHA256(bot_token)` per Telegram's own spec) verified in `miniapp/auth.py`'s `auth_middleware`, sent by the frontend as an `X-Telegram-Init-Data` header on every `fetch()` (never as a URL param — Telegram doesn't append anything to the WebApp URL itself, the frontend reads `Telegram.WebApp.initData` client-side). Resolves to an `Employee` via `telegram_id`, same identity source as the chat's `RoleAccessMiddleware`. Role-gating is done at the **sub-app** level, not per-handler decorators: `/api/miniapp/admin`, `/brigadier`, `/seller` are separate nested `aiohttp.web.Application`s each with their own `role_middleware(...)`, mounted under the parent `api_app` (which only requires "is an active employee") — `request["employee"]` set by the parent's `auth_middleware` propagates through to sub-app middlewares since it's the same `Request` object. `common`/`worker` routes carry no role gate — worker actions rely on task *assignment*, not role, matching the chat handlers' own lack of a role check there.
- **Frontend** (`miniapp/public/`, vanilla JS/CSS, no build step, no framework): reuses the web panel's validated design tokens as a **static copy** (`public/css/app.css` — bot can't import `web/`'s CSS across languages) since Telegram WebApp messages can't be styled any other way. Navigation is a JS-side stack (`nav.stack`) driving Telegram's native `BackButton`; the primary action per screen is always Telegram's native `MainButton` (never an in-page "submit" button) — `setMainButton()`/`hideMainButton()` wrap it. Real per-screen dynamic content (task titles, employee names, department names) is **not** translated — only UI chrome (labels, statuses, buttons) is, via `public/js/i18n.js`'s `uz`/`ru` dictionary, because those are free text an admin typed in, not strings this system owns a translation for.
...
- **New task creation is a single scrollable form**, not the 5-step wizard shown in the approved mockup — a deliberate scope cut to ship the write path correctly rather than half-build a stepper UI; revisit if the flatter form proves confusing in practice.
- **Deploy note**: `config.settings.port` (env `PORT`, default `8080`) is the Mini App's listen port; `config.settings.miniapp_base_url` (env `MINIAPP_BASE_URL`) must be a public HTTPS URL for the `web_app` button / `bot.set_chat_menu_button()` call in `main.py` to activate at all — both are skipped silently (not an error) when unset, so the bot keeps working chat-only in local dev with no public domain. Getting a real HTTPS URL (Railway public domain on the existing bot service, or a tunnel for a quick test) is still an open deploy step, not yet done.
...
- `utils/formatters.py` is the shared home for cross-handler display logic: `format_dt()`, `TASK_STATUS_LABELS`, and `format_task_card(task)` (one task's status/deadline/days-remaining block, reused by every worker-facing task listing). Add new shared formatting here rather than re-deriving it per-handler.
- Worker-facing task views are split by `TaskType`, not merged: `/tasks` (`mytasks`) lists `ORDER` tasks only, `/misctasks` (`mymisc`) lists `MISC` only, `/myscore` (`mypoints`) shows the worker's own current-month KPI log (via `penalty_service.month_bounds()` — the public rename of what was `_month_bounds` — and `KpiLogRepository.list_by_employee_in_range()`, newest-first). `/start` attaches an inline menu (`keyboards/worker_kb.build_worker_menu_keyboard()`) to these three for `Role.WORKER` only; the callback handlers for that menu live in `handlers/worker/tasks.py` alongside the slash commands, not in `handlers/common/start.py`. `notification_service.notify_task_started` attaches the same `build_task_keyboard(task)` used by `/tasks` — a new task-assignment DM is actionable immediately, not just informational.
